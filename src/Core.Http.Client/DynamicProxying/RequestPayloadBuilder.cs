...
namespace Core.Http.Client.DynamicProxying
{
    ...
    {
        [CanBeNull]
        public static HttpContent BuildContent(ActionApiDescriptionModel action, IReadOnlyDictionary<string, object> methodArguments, ApiVersionInfo apiVersion)
        {
            var body = GenerateBody(action, methodArguments);
            if (body != null)
            {
        ...
        private static string GenerateBody(ActionApiDescriptionModel action, IReadOnlyDictionary<string, object> methodArguments)
        {
